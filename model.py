...
class Model(nn.Module):
    def __init__(self, feature_dim=128, model_select="resnet50", width=64):
        super(Model, self).__init__()
        if model_select == "resnet10vw":
            proj_in = width*8
            base_model = resnet10vw(width)
        elif model_select == "resnet50":
            width = 64
            proj_in = 2048
            base_model = resnet50()
        else:
            raise ValueError("Need to specify a model name. Choice: ")
        proj_in2 = proj_in//4
        self.f = []
        for name, module in base_model.named_children():
            if name == 'conv1':
                module = nn.Conv2d(3, width, kernel_size=3, stride=1, padding=1, bias=False)
            if not isinstance(module, nn.Linear) and not isinstance(module, nn.MaxPool2d):
                self.f.append(module)
        # encoder
        self.f = nn.Sequential(*self.f)
        # projection head
        self.g = nn.Sequential(nn.Linear(proj_in, proj_in2, bias=False), nn.BatchNorm1d(proj_in2),
                               nn.ReLU(inplace=True), nn.Linear(proj_in2, feature_dim, bias=True))
    ...
    def forward(self, x):
        ...
        # print(f"Called flatten. Feature is: {feature.shape}")
        out = self.g(feature)
        # print("Called g")
